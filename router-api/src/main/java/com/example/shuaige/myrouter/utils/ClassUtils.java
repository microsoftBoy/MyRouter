package com.example.shuaige.myrouter.utils;
...
public class ClassUtils {
            ...
            DefaultPoolExecutor.getInstance().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Log.e("MyRouter", "Scan map file in dex files made error.", ignore);
                    } finally {
                        if (null != dexfile) {
                            try {
                                dexfile.close();
    ...
    public static List<String> getSourcePaths(Context context) throws PackageManager.NameNotFoundException, IOException {
        ...
        Log.i(TAG, "getSourcePaths: applicationInfo.sourceDir = " + applicationInfo.sourceDir);
        //applicationInfo.sourceDir = /data/app/com.example.shuaige.myrouter-1/base.apk
        File sourceApk = new File(applicationInfo.sourceDir);
        List<String> sourcesPaths = new ArrayList<>();
        sourcesPaths.add(applicationInfo.sourceDir);//添加默认的apk路径

        //提取文件前缀
        String extractedFilePrefix = sourceApk.getName() + EXTRACTED_NAME_EXT;
        Log.i(TAG, "getSourcePaths: extractedFilePrefix = " + extractedFilePrefix);

        if (!isVMMultidexCapable()) {
            int totalDexNumber = getMultiDexPreferences(context).getInt(KEY_DEX_NUMBER, 1);
            File dexDir = new File(applicationInfo.dataDir, SECONDARY_FOLDER_NAME);
            Log.i(TAG, "getSourcePaths: dexDir = " + dexDir);
            for (int secondaryNumber = 2; secondaryNumber < totalDexNumber; secondaryNumber++) {
                String fileName = extractedFilePrefix + secondaryNumber + EXTRACTED_SUFFIX;
                File extractedFile = new File(dexDir, fileName);
                if (extractedFile.isFile()) {
                    sourcesPaths.add(extractedFile.getAbsolutePath());
                } else {
        ...
        sourcesPaths.addAll(tryLoadInstantRunDexFile(applicationInfo));
        return sourcesPaths;
    ...
    private static boolean isVMMultidexCapable() {
        ...
        try {
            if (isYunOS()) {
                ...
                Log.i(TAG, "isVMMultidexCapable: vmVersion = " + vmVersion);
                if (vmVersion != null) {
